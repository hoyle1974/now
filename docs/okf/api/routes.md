...
| `GET /todos/trash` (one entry per trashed item, with `deleted_with`, `trashed_at`), `POST /todos/clear-completed`, `PATCH /todos/{id}/undelete` | [Trash](../features/trash-archive.md). |
