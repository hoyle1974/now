...
**Open sheets survive re-renders.** The Add / Split / Edit sheets live inside the tree, and `renderTree()` rebuilds the whole tree on every change (a sync ack, folding a subtask, a refresh). `web/tree-view.js` snapshots the open sheet's field values, focus and caret before the rebuild and restores them after (`snapshotSheet` / `restoreSheet`), only if the same panel (mode + todo id) is still open. Color and link edits are JS state, not fields, and are not carried over.
