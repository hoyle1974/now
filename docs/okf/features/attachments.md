...
- **Bytes** live in the private bucket `<project-id>-attachments` (`us-central1`, public access prevention on, uniform access, no CORS) at `users/{email}/todos/{todo_id}/{attachment_id}` (partitioned per signed-in user, `app/blobstore.py:key_for`/`user_todos_prefix`, [multi-user](../ops/multi-user.md)). `app/blobstore.py` has `GcsStore` and an in-memory store; the bucket is chosen by the `ATTACHMENTS_BUCKET` env var. Unset locally = in-memory (lost on restart); unset on Cloud Run (`K_SERVICE`) = uploads fail with an error rather than lose data. Provision with `scripts/create-bucket.sh` ([deploy](../ops/deploy.md)).
...
- **Client:** `web/attachments.js` (pure helpers) and `web/attachments-ui.js` (the "Images" section, shown in the todo viewer and in the Edit sheet: picker, drag-drop, paste, lightbox; registry field `attachments`, so every type has it; the New item sheet has none because a todo must exist first). Images upload as soon as they are added, independent of Save, and a title typed in the open Edit sheet survives the upload. Images are fetched with the auth token and shown as blob URLs. Online only: nothing is queued in the outbox, a todo with a `tmp:` id can't take images, and the UI adopts the response's `attachments`/`version` and reports its rev via `engine.observeRev` ([sync](sync-model.md)).
