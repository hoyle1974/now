...
**Picking a date in the edit sheet:** the chip row is Today / Tomorrow / Pick date / Clear. "Pick date" opens the native picker (`showPicker()`, falling back to focus) and shows the chosen date once it is neither Today nor Tomorrow; tapping the date field opens the picker too. An empty date field is blank on iOS, hence the explicit chip (the composer has the same chip).
