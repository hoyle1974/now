...
- **Trash** (`web/trash.js`): its own screen (the List / Next up control is hidden, accent-colored Back button); `GET /todos/trash`; restore with `undelete`. If an ancestor is also deleted the todo restores at top level.
