...
| `title`, `done`, `create_date` | Basics. Titles are capped at 2000 chars on write (split: 100 items); `create_date` is naive UTC (`models.utc_now`) in storage but sent to clients as an explicit UTC instant (`...Z`); `due_date` stays bare (the user's wall-clock time, not an instant). |
