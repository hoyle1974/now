...
- `.now.env` (git-ignored; `.now.env.example`): app-side overrides `SERVICE`, `HOSTING_SITE`, `ALLOWED_EMAILS`, `PROJECT`, `REGION`. The service is `now-app`, which is also zilch's `app_name`, so neither `SERVICE` nor `HOSTING_SITE` needs pinning; keep the file only for genuine overrides.
...
- `ALLOWED_EMAILS` is required (`;`-separated, first is the owner; legacy single-value `ALLOWED_EMAIL` still works, [stack](../architecture/stack.md)); `scripts/scriptable-next-up.js` has a `BASE` placeholder.
