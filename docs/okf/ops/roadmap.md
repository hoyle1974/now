...
3. **Delete the old `now` service** once the Scriptable `BASE` is `https://now-app.web.app` (`gcloud run services delete now --region us-central1`), and keep an eye on the first days of `now-app - health check failing`. Done 2026-09-20: migration, monitoring apply, old alerts removed.
...
6. **Scrub the author's project id / URLs** from README history and this OKF bundle if the repo goes public (README is already clean; `.firebaserc`, `firebase.json` and OKF still name the project).
